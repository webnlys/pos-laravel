<template>
    <PageShell :title="id ? 'Edit supplier' : 'New supplier'">
...
onMounted(async () => {
    if (!id) return;
    const { data } = await axios.get(`/api/admin/suppliers/${id}`);
...
async function save() {
    error.value = '';
    try {
        if (id) await axios.put(`/api/admin/suppliers/${id}`, form);
        else await axios.post('/api/admin/suppliers', form);
